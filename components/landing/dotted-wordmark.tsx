interface DottedWordmarkProps {
  text: string;
  className?: string;
  /** Malha mais densa e shimmer mais marcado (ex.: sobre o cartão na v4). */
  variant?: "default" | "letter";
}

/**
 * Texto preenchido por uma malha de pontos, com brilho que desliza lentamente.
 * O texto fantasma abaixo mantém o espaço no fluxo e serve de fallback acessível.
 */
export function DottedWordmark({
  text,
  className = "",
  variant = "default",
}: DottedWordmarkProps) {
  const markClass =
    variant === "letter" ? "dotted-wordmark dotted-wordmark--letter" : "dotted-wordmark";

  return (
    <span className={`relative inline-block select-none ${className}`.trim()}>
      <span className="invisible" aria-hidden>
        {text}
      </span>
      <span className={`${markClass} absolute inset-0 flex items-center justify-center whitespace-nowrap`}>
        {text}
      </span>
    </span>
  );
}
